...
/**
 * v4.8.4 regression-safe UI stabilization.
 * IMPORTANT: this class never overwrites functional View tags used by v4.7/v4.8.
 * That prevents the repeated Save Draft regression caused by earlier styling patches.
 */
public class OfferStudioV484Activity extends OfferStudioV480Activity {
    ...
    @Override protected void onResume() {
        ...
        if (!running484) {
            running484 = true;
            ui484.postDelayed(polish484, 180);
        }
    }

    @Override protected void onPause() {
        running484 = false;
        ui484.removeCallbacks(polish484);
        super.onPause();
    }
    ...
    private void bindImmediateLanguage(View root) {
        ...
        Spinner spinner = findLanguageSpinner(root);
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            @Override public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String next = languageForPosition(parent, position);
                String current = lang();
        ...
        if (v instanceof Spinner) {
            ...
            if (s.getAdapter() != null) {
                ...
                for (int i=0;i<s.getAdapter().getCount();i++) {
                    ...
                    if (x.contains("کورد") || x.contains("Kurd")) ku=true;
                }
    ...
    private void hideBlankImages(View v) {
        if (v instanceof ImageView) {
            ...
            if (d==null && im.getVisibility()==View.VISIBLE) {
                ...
                if (lp!=null && lp.width>dp(55) && lp.height>dp(55)) {
    ...
    /* ---------------- stable button sizing, tags untouched ---------------- */
    private void stabilizeExistingButtons(View v) {
        if (v instanceof Button) {
            Button b=(Button)v;
            if(!styled.contains(b)){
                styleButtonWithoutChangingTag(b);
                styled.add(b);
            }
        }
    ...
    private void styleButtonWithoutChangingTag(Button b){
        ...
        boolean regular = main || danger || settingsSmall
                ...
                || s.contains("organizer") || s.contains("منظم") || s.contains("ڕێکخەر")
                || s.contains("photos") || s.contains("صور") || s.contains("وێنە");
        if(!regular)return;

        b.setAllCaps(false);
        b.setTextSize(settingsSmall?11.5f:12.5f);
        ...
        if(raw instanceof LinearLayout.LayoutParams){
            LinearLayout.LayoutParams lp=(LinearLayout.LayoutParams)raw;
            if(settingsSmall)lp.height=dp(40);
            else if(main)lp.height=dp(50);
            else lp.height=dp(44);
